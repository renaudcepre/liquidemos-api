...
def test_project_model(create_project):
...
@pytest.mark.django_db
def test_tree_grand_childs(project_tree):
...
@pytest.mark.django_db
def test_retrieve_parents(project_tree):
...
@pytest.mark.django_db
def test_vote(admin_user, create_user, create_project):
...
def test_delegation(create_user):
    ...
    assert Delegation.get_delegations(delegate, tag).count() == 1
    assert Delegation.get_delegations(delegator, tag).count() == 0
